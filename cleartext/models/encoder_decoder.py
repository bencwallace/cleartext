...
class Encoder(nn.Module):
    def __init__(self, embed_weights, units, dropout):
        ...
        self.rnn = nn.GRU(embed_dim, units, bidirectional=True)
...
class Attention(nn.Module):
    ...
    def forward(self, dec_state, enc_outputs):
        ...
        # vectorize computation of attention scores for all encoder outputs
        dec_state = dec_state.unsqueeze(1).repeat(1, source_len, 1)
        enc_outputs = enc_outputs.permute(1, 0, 2)
        scores = torch.tanh(self.fc(torch.cat((dec_state, enc_outputs), dim=2)))
...
class Decoder(nn.Module):
    ...
    def forward(self, sequence, dec_state, enc_outputs):
